...
options:
  vm_name:
    description:
      - The name of the vSphere VM to Instantly Recover.
    required: True
    type: str
  date:
    description:
      - The date of the snapshot you wish to Instantly Recover formated as `Month-Day-Year` (ex: 1-15-2014). If 'latest' is specified, the last snapshot taken will used. 
    required: False
    type: str
    default: latest
  time:
    description:
      - The time of the snapshot you wish to Instantly Recover formated formated as `Hour:Minute AM/PM`  (ex: 1:30 AM). If 'latest' is specified, the last snapshot taken will be used. 
    required: False
    type: str
    default: latest
  host:
  ...
  power_on:
    description:
      - Flag that determines whether the VM should be powered on after Instant Recovery. Set to `True` to power on the VM. Set to `False` to instantly recover the VM but not power it on.
    required: False
    type: bool
    default: True
  disable_network:
    description:
      - Sets the state of the network interfaces when the VM is instantly recovered. Use `False` to enable the network interfaces. Use `True` to disable the network interfaces. Disabling the interfaces can prevent IP conflicts. 
    required: False
    type: bool
    default: False
  keep_mac_addresses:
    description:
      - Flag that determines whether the MAC addresses of the network interfaces on the source VM are assigned to the new VM. Set to `True` to assign the original MAC addresses to the new VM. Set to `False` to assign new MAC addresses. When 'remove_network_devices' is set to `True`, this property is ignored. 
      required: False
      type: bool
      default: False
  preserve_moid:
    description:
      - Flag that determines whether to preserve the MOID of the source VM in a restore operation. Use `True` to keep the MOID of the source. Use `False` to assign a new moid.
    required: False
    type: bool
    default: False
  timeout:
...
RETURN = '''
version: 
    description: The full response for POST /v1/vmware/vm/snapshot/{snapshot_id}/instant_recover.
    returned: success
    type: dict
'''
